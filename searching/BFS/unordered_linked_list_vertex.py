...
#Purpose   : Unordered Linked List optimized for Adjecency List 
#Author    : Atul Kumar
#Created   : 29/07/2016
#License   : GPL V3
...
class Node:
	...
	def __init__(self,key): # Constructor 
		self.key = key
		self.edge = None #Edge List
		self.next = None #Next vertex node
		self.color = None
		self.distance = None
		self.parent = None 

	def get_key(self): # Return the key of node
		return self.key

	def get_edge(self): # Return the edge list
		return self.edge

	def get_next(self): # Return the reference of next node linked with it
		return self.next

	def get_color(self): #Return the color of the vertex
		return self.color

	def get_distance(self): # Return the shortest distance of vertex form the source
		return self.distance

	def get_parent(self): # Return the reference of parent 
		return self.parent

	def set_key(self,new_key):
		self.key = new_key

	def set_edge(self,new_edge): #setter for edge List
		self.edge=new_edge
...
class UnorderedList:
	...
	def __str__(self):
		...
		if current != None:
			lview=""
			while(current != None):
				lview=lview + str(current.get_key()) + "->" + str(current.get_edge()) + "\n"
	...
	def add(self,key): # adds an item at the beginning of the list
		t = Node(key)
	...
	def append(self,key): # append an item to the end of the linked list
		current = self.head
		if self.head != None:
	...
	def insert(self,pos,key): #insert an item at specified position in the linked list
		current = self.head
		count = -1
