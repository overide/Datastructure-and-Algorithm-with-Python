...
#Purpose   : Unordered Linked List optimized of Adjacency List 
#Author    : Atul Kumar
#Created   : 29/07/2016
#License   : GPL V3
...
class Node:
	...
	def __init__(self,key): # Constructor need key to initilize the node
		self.key = key
		self.next = None
	...
	def set_key(self,new_key): #setter for key
		self.key=new_key
		...
		if current != None:
			...
			while(current != None):
				if current.get_next() != None:
					lview=lview + str(current.get_key().get_key())+"->"
				else:
					lview=lview + str(current.get_key().get_key())
